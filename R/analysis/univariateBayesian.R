######## 
source("R/functions/partialPredBayes.R")
...
foreach.results <- foreach(i = 1:nrow(guide),
                           .packages = c('dplyr', 'brms', 'mgcv', 'broom', 'ggplot2', 'tidyr', 'data.table', 'Metrics', 'tidyverse', 'MuMIn' ,"MetBrewer", 
                                         'grid', 'gridExtra','loo'),
                           .options.snow = opts,
                           .inorder = FALSE,
                           .combine = rbind.lists) %dopar% {
                             ...
                             #models
                             m0 <- tryCatch(
                               {brm(intercept_only_formula, data = dt.sub)},
                             ...
                             if(is.null(m)){return(NULL)}else{
                               ...
                               deltaWaic <- WAIC(m0)$estimates[3] - WAIC(m)$estimates[3]
                               deltaLoo <- loo::loo(m0)$estimates[3] - loo::loo(m)$estimates[3]
                               
                               
                               
                               tmp <- tmp %>% 
                                 mutate(
                                   r_squared = bayes_R2(m)[1], 
                                   waic = WAIC(m)$estimates[3], 
                                   loo = loo::loo(m)$estimates[3],
                               ...
                               #bring in good shape 
                               tmp.est <- tidy.m %>% 
                                 rename(ci.ub = conf.high, 
                                        ci.lb = conf.low) %>% 
                                 mutate(response_tier = tier, 
                                        scale = scale, 
                                        formula_id = formula.id, 
                                        response = filter.resp, 
                                        sig = ifelse(ci.lb > 0 | ci.ub < 0, "significant", "non significant"), 
                               ...
                               if(guide[i, ]$interaction == FALSE){
                                 ...
                                 for(j in 1:nrow(var.names)){
                                   ...
                                   marg.tmp <- partialPredBayes(model = m, response = filter.resp, var = var,
