...
dt.est <- estimates %>% 
  ...
  mutate(
    ...
    clean_term = case_when(
      ...
      term %in% c("nEventsDayReserve_scaled", "nEventsDay_scaled") ~ "Herbivore Visitation"), 
    ...
    clean_response = case_when(
      ...
      #Resilience
      response %in% c("plot_max_cover_ms", "site_max_cover_ms", "reserve_max_cover_ms") ~ "Community Dominance",
  ...
  mutate(
    ...
    sig_pn = case_when(
      ...
      ci.lb > 0 ~ "Positive estimate;\nCI not overlapping 0",
      ci.ub < 0 ~ "Negative estimate;\nCI not overlapping 0"),
...
dt.est.div <- dt.est[response_tier == "Diversity"] %>% 
  filter(clean_term %in% c("MAP", "Herbivore Species Richness", "Herbivore Biomass", "Herbivore Visitation")) %>%
  filter(interaction == FALSE)  %>% 
...
dt.est.div <- dt.est.div %>%
  complete(scale_n, nesting(clean_term, clean_response),
           fill = list(estimate = NA, ci.lb = NA, ci.ub = NA, 
                       sig_pn = "CI overlapping 0", better_than_intercept = "Similar to Null-Model"))  %>% 
...
p.div1 <- dt.est.div %>% 
  ...
  scale_fill_manual(values=c("CI overlapping 0" = "grey50",
                             "Positive estimate;\nCI not overlapping 0" =  "#E17C29",
                             "Negative estimate;\nCI not overlapping 0" = "#3E1E62"), guide = "none") + 
  scale_color_manual(values=c("CI overlapping 0" = "grey50",
                              "Positive estimate;\nCI not overlapping 0" =  "#E17C29",
...
p.div.raw <- gridExtra::grid.arrange(p.div1, p.div2, p.div3, heights = c(1.3, 1, 1))
p.div <- gridExtra::grid.arrange(p.div.raw, div.leg, heights = c(5, 1)) 

ggsave(plot = p.div, "builds/plots/diversityGridGLMM_bonferroni.png", dpi = 600, height =9, width = 12)
...
p.lfd.raw <- gridExtra::grid.arrange(p.lfd1, p.lfd2, p.lfd3, heights = c(1.3, 1, 1))
p.lfd <- gridExtra::grid.arrange(p.lfd.raw, lfd.leg, heights = c(5, 1))


ggsave(plot = p.lfd, "builds/plots/lifeFormDivGridGLMM_bonferroni.png", dpi = 600, height = 8, width = 12)
...
p.resi1.5 <- dt.est.resi %>%
  filter(clean_response == "Community Dominance") %>%  
  mutate(clean_response = "Community\nDominance") %>%  
  ggplot() +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey25", alpha = 0.75, linewidth = .5) +
  geom_pointrange(aes(x = estimate, xmin = ci.lb, xmax = ci.ub, y = clean_term,
                      color = sig_pn, fill = sig_pn,  shape = better_than_intercept),
...
p.resi.raw <- gridExtra::grid.arrange(p.resi1.5, p.resi2, p.resi3, heights = c(1.3, 1, 1))
p.resi <- gridExtra::grid.arrange(p.resi.raw, resi.leg, heights = c(6, 1))


ggsave(plot = p.resi, "builds/plots/resilienceGridGLMM_bonferroni.png", dpi = 600, height = 10, width = 9)
...
str.leg <- ggpubr::get_legend(p.str.leg, position = "bottom")
as_ggplot(str.leg)

p.str.raw <- gridExtra::grid.arrange(p.str1, p.str2, p.str3, heights = c(1.3, 1, 1))
p.str <- gridExtra::grid.arrange(p.str.raw, str.leg, heights = c(5, 1))

ggsave(plot = p.str, "builds/plots/structureGridGLMM_bonferroni.png", dpi = 600, height = 10, width = 7)
...
##### Div------------
dt.pred.div <- dtPred %>%
  filter(response_tier == "Diversity" & grepl("CI not", sig_pn) & !grepl("MAP", term))
...
ggsave(plot = reserve.leg, "builds/plots/reserveLeg_bonferroni.png", dpi = 600, height =2, width = 10)
...
dt.pred.lfd <- dtPred %>%
  filter(response_tier == "Life Form Specific Diversity" & grepl("CI not", sig_pn) & !grepl("MAP", term))
...
dt.pred.resi <- dtPred %>%
  filter(response_tier == "Resilience" & grepl("CI not", sig_pn) & !grepl("MAP", term)) %>% 
  filter(clean_term %in% c("MAP", "Herbivore Species Richness", "Herbivore Biomass", "Herbivore Visitation") &
           clean_response != "Functional Beta Diversity") %>% filter(interaction == FALSE)
...
ggsave(plot = gridComb, "builds/plots/gridTest_bonferroni.png", width = 12.3, height = 13.8)
...
# Figure 2 ---------------------
p_div_new <- gridExtra::grid.arrange(p.div1, p.div2, p.div3, heights = c(1.3, 1, 1))

p_lfd_new <- gridExtra::grid.arrange(p.lfd1 + theme(axis.text.y = element_blank()),
                                     p.lfd2 + theme(axis.text.y = element_blank()),
                                     p.lfd3 + theme(axis.text.y = element_blank()),
                                     heights = c(1.3, 1, 1))


p_fig2_ab <- gridExtra::grid.arrange(p_div_new, p_lfd_new, widths = c(1.5, 1))
p_fig2_ab_leg <- gridExtra::grid.arrange(p_fig2_ab, lfd.leg, heights = c(6, 1))

ggsave(plot = p_fig2_ab_leg, "builds/plots/fig2_bonferroni.png", dpi = 600, height = 8, width = 12)


# Figure 3 -------
grid_div <- gridExtra::grid.arrange(p.div.trends[[1]], p.div.trends[[2]],
                                    p.div.trends[[3]], p.div.trends[[4]],
                                    p.div.trends[[5]],
                                    ncol = 5)

grid_lfd <- gridExtra::grid.arrange(p.lfd.trends[[1]], p.lfd.trends[[2]],
                                    p.lfd.trends[[3]], p.lfd.trends[[4]], 
                                    p.lfd.trends[[5]], p.lfd.trends[[6]],
                                    p.lfd.trends[[7]], p.lfd.trends[[8]], 
                                    p.lfd.trends[[9]], p.lfd.trends[[10]],
...
p_fig3 <- gridExtra::grid.arrange(grid_div, empty_plot, grid_lfd, reserve.leg,
                                  heights = c(1, 0.2, 2, 0.5))

ggsave(plot = p_fig3, "builds/plots/fig3_bonferroni.png", dpi = 600, height = 9, width = 12)


### Figure 4 -------------

p_resi_raw <- gridExtra::grid.arrange(p.resi1.5,
                                      p.resi2, 
                                      p.resi3,
                                      resi.leg,
                                      heights = c(1.3, 1, 1, 0.5))

p_fig4 <- gridExtra::grid.arrange(p_resi_raw, empty_plot, widths = c(1.5, 1))


ggsave(plot = p_fig4, "builds/plots/fig4_bonferroni.png", dpi = 600, height = 8.5, width = 12)

### Figure 5 -----------------
grid_resi <- gridExtra::grid.arrange(p.resi.trends[[1]], p.resi.trends[[2]],
                                     p.resi.trends[[3]], p.resi.trends[[4]],
                                     p.resi.trends[[5]], p.resi.trends[[6]],
                                     p.resi.trends[[7]], 
                                     ncol = 4)
...
p_fig5 <- gridExtra::grid.arrange(grid_resi, reserve.leg, ncol = 1, heights = c(5, 1))


ggsave(plot = p_fig5, "builds/plots/fig5_bonferroni.png", dpi = 600, height = 6, width = 10)


#### export p values and stuff....
...
dt_exp <- dt.est %>% 
  ...
  dplyr::select(clean_response, clean_term, scale, estimate_and_ci, p.value, p.adj, R2m, R2c) %>% 
...
fwrite(dt_exp, "builds/modelOutputs/glmm_estimates_cleanish.csv")
